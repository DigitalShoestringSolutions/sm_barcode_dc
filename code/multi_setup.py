...
if __name__ == "__main__":
    try:
        ...
        if conf["module_enabled"] != True:
            ...
            if not setup:
                all_devices = bind_all(multi_barcode_scan.DeviceManager.get_udev_context())
                if len(all_devices) == 0:
                    print_output("No barcode scanners detected - please connect and try again", variant="error")
                    exit(0)
